 ...
 * @out_bw_max: pointer to store the maximum (allowed) performance level,
 *		as required by cpufreq_driver_adjust_perf().
 */
...
{
	unsigned long min = 0, max = 0, util = cpu_util_cfs(cpu);
	if (cpu_util_cfs_boost(cpu) > util) util = cpu_util_cfs_boost(cpu);
	util = effective_cpu_util(cpu, util, ENERGY_UTIL, NULL);
	util = max(util, boost);
	*out_bw_min = min;
	*out_bw_max = max;
	*out_util = util;
